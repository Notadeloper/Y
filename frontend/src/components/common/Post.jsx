...
const Post = ({ post }) => {
    ...
    const { mutate: likePost, isPending: isLiking } = useMutation({
        ...
        onSuccess: (updatedLikes) => {
            // Following is not good UX
            // queryClient.invalidateQueries({queryKey: ["posts"]});
            // SO instead, we directly update the cache for that post

            queryClient.setQueryData(["posts"], (oldData) => {
                return oldData.map(p => {
                    if (p._id === post._id) {
                        // Only for the particular post it replaces likes with updatedLikes
    ...
    const { mutate: bookmarkPost, isPending: isBookmarking } = useMutation({
        mutationFn: async () => {
            const res = await fetch(`/api/users/bookmarks/${post._id}`, {
                method: "POST",
            });
            const data = await res.json();
            if (!res.ok) {
        ...
        onSuccess: (updatedBookmarkedPosts) => {
            queryClient.setQueryData(["authUser"], (oldUser) => {
                if (!oldUser) return oldUser;
                return { ...oldUser, bookmarkedPosts: updatedBookmarkedPosts};
            });
        },
        onError: (error) => {
            toast.error(error.message);
				...
				<div className='flex flex-col flex-1'>
					...
					<div className='flex items-center justify-between mt-3'>
						...
						<div className='flex gap-1 items-center group cursor-pointer' onClick={handleBookmarkPost}>
							...
							{isBookmarked && !isBookmarking && (
								<FaRegBookmark className='w-4 h-4 text-sky-400 cursor-pointer group-hover:text-slate-500' />
							)}
						</div>
					</div>
				</div>
